...
class BaseSensorSimTask():
	"""
	An Abstract class to generate common sensor simulator functionality that can be used for every sensor.
	...
	def __init__(self, name = ConfigConst.NOT_SET, typeID: int = ConfigConst.DEFAULT_SENSOR_TYPE, \
			dataSet = None, minVal: float = DEFAULT_MIN_VAL, maxVal: float = DEFAULT_MAX_VAL):
	...
	def generateTelemetry(self) -> SensorData:
		...
		if self.useRandomizer:
			...
			#incrementing the dataset index and if its at last then resetting it to 0 position
